function tSeq = SequentialProcessing(FileName)
...
LogFileName = 'SequentialResults.txt';
LogID = fopen('SequentialResults.txt', 'a');
...
fprintf(LogID, '%s: Total time for sequential processing with %i of data = %.2f s\n\n', datestr(now, 0), size(Data2Process,1), tSeq);
fclose(LogID);
end